...
#define NCG_OP_DEF_NAME(op_name_) virtual const char *op_name(void) const { return #op_name_; }
    ...
    if (inputs.size() > 0) { \
        for (ssize_t i = 1; i < inputs.size(); ++i) { \
            if (inputs[i]->desc().dtype() != inputs[0]->desc().dtype()) { \
                auto &err_stream = ctx.error(this) << op_name() << " requires all inputs to have the same dtype, but got: "; \
    ...
    if (inputs.size() > 0) { \
        for (ssize_t i = 1; i < inputs.size(); ++i) { \
            if (inputs[i]->desc().dim() != inputs[0]->desc().dim()) { \
                auto &err_stream = ctx.error(this) << op_name() << " requires all inputs to have the same dimension, but got: "; \
                for (ssize_t j = 0; j < inputs.size(); ++j) { \
                    if (j != 0) err_stream << ", "; \
    ...
    if (inputs.size() > 0) { \
        for (ssize_t i = 1; i < inputs.size(); ++i) { \
            if (!inputs[i]->desc().is_compatible(inputs[0]->desc())) { \
                auto &err_stream = ctx.error(this) << op_name() << " requires all inputs to have compatible shapes, but got: "; \
    ...
    if (inputs.size() > 0) { \
        for (ssize_t i = 1; i < inputs.size(); ++i) { \
            if (!inputs[i]->desc().is_compatible(inputs[0]->desc(), true)) { \
                auto &err_stream = ctx.error(this) << op_name() << " requires all inputs to have broadcastable shapes, but got: "; \
                NCG_OP_CHECK_COMPATIBLE_SHAPE_PRINT(ctx, inputs, err_stream); \
                err_stream << "."; \
                return; \
            } \
...
#define NCG_OP_CHECK_INPUT_DTYPE(ctx, inputs, idx, dtype_name) do { \
    ...
    if (inputs[idx_value]->desc().dtype() != DTypeName::dtype_name) { \
        ctx.error(this) << op_name() << " requires the input " << (idx_value + 1) << " to have dtype " << #dtype_name << ", but got " << get_dtype_name(inputs[idx_value]->desc().dtype()) << "."; \
        return; \
    } \
} while (0)
...
#define NCG_OP_CHECK_INPUT_DTYPE_INT(ctx, inputs, idx) do { \
    ...
    if (inputs[idx_value]->desc().dtype() != DTypeName::Int32 && inputs[idx_value]->desc().dtype() != DTypeName::Int64) { \
        ctx.error(this) << op_name() << " requires the input " << (idx_value + 1) << " to have dtype Int32 or Int64, but got " << get_dtype_name(inputs[idx_value]->desc().dtype()) << "."; \
        return; \
    } \
} while (0)
...
#define NCG_OP_CHECK_INPUT_DTYPE_FLOAT(ctx, inputs, idx) do { \
    ...
    if (inputs[idx_value]->desc().dtype() != DTypeName::Float32 && inputs[idx_value]->desc().dtype() != DTypeName::Float64) { \
        ctx.error(this) << op_name() << " requires the input " << (idx_value + 1) << " to have dtype Float32 or Float64, but got " << get_dtype_name(inputs[idx_value]->desc().dtype()) << "."; \
        return; \
    } \
} while (0)
...
#define NCG_OP_CHECK_INPUT_DIM(ctx, inputs, idx, dim_expr) do { \
    ...
    if (inputs[idx_value]->desc().dim() != dim_value) { \
        ctx.error(this) << op_name() << " requires the input " << (idx_value + 1) << " to have dimension " << dim_value << ", but got " << inputs[idx_value]->desc().dim() << "."; \
        return; \
    } \
} while (0)
...
#define NCG_OP_CHECK_INPUT_SCALAR(ctx, inputs, idx) do { \
    ...
    if (inputs[idx_value]->desc().dim() != 0) { \
        ctx.error(this) << op_name() << " requires the input " << (idx_value + 1) << " to be a scalar, but got a " << inputs[idx_value]->desc().dim() << " dimensional input."; \
        return; \
    } \
} while (0)
...
#define NCG_OP_CHECK_INPUT_VECTOR(ctx, inputs, idx) do { \
    ...
    if (inputs[idx_value]->desc().dim() != 1) { \
        ctx.error(this) << op_name() << " requires the input " << (idx_value + 1) << " to be a vector, but got a " << inputs[idx_value]->desc().dim() << " dimensional input."; \
        return; \
    } \
} while (0)
